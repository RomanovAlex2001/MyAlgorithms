...
void group_update(node * root, int left, int right, int delta) {
    if (right < root->left || left > root->right) {
        return;
    }
    if (right >= root-> right && left <= root->left) {
    ...
    group_update(root->child_left, left, right, delta);
    group_update(root->child_right, left, right, delta);
    root->maxi = max(root->child_left->maxi + root->child_left->add,
                     root->child_right->maxi + root->child_right->add);
}
